...
import spring.java.io.shop.notification.email.Email;
...
public class PostalService {
	...
	protected Session getSession() {
		if(session==null) {
			...
			properties.put("mail.smtp.port", emailTransportConfig.getSmtpPort());
	...
	protected Message createMessage(Email email) throws MessagingException{
		...
		for(String to:email.getToAddress()) {
		...
		for (String bcc : email.getBccAddress()) {
	...
	protected String getProtocol() {
		...
		if(emailTransportConfig.isUseSecureSmtp()) {
			protocol="smtps";
		}
		return protocol;
	}
